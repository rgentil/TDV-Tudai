...
public class MercadoLibreBuscarProducto {
	...
	public void search(String busqueda) {
		...
		WebElement searchbar = (WebElement) wait
				.until(ExpectedConditions.visibilityOfElementLocated(By.className("nav-search-input")));
		searchbar.clear();
		...
		while (findElement == false) {
			...
			try {
				...
				WebElement element = (WebElement) wait
						.until(ExpectedConditions.visibilityOfElementLocated(By.partialLinkText(textoABuscar)));
				findElement = true;
				element.click();
			} catch (Exception e) {
				findElement = false;
